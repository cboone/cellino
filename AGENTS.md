...
- **A check that passes by finding nothing is not coverage.** The shell job does exactly that before Phase 1, and a local `actionlint` without shellcheck on `PATH` does it always. Say so rather than counting it. See [CI workflows](docs/notes/ci-workflows.md).
