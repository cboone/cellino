...
**Subnormals are handled in the register, not in the float mode.** After a release, a reed's ring-down passes minus 60 dB, the audible end of the note, within seconds, but minus 60 dB is nowhere near underflow: the state keeps decaying exponentially, and with nothing to flush it, every release eventually drives it into the subnormal range, where arithmetic slows sharply, long after it has stopped being audible. Flush-to-zero is set through bit 24 of the ARM64 floating-point control register, saved and restored around `process()`, because `@setFloatMode` does not touch that register. A runtime check the optimizer cannot fold confirms it, verified by plant.
