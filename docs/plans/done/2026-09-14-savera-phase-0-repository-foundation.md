...
Before this plan was committed, the repository held six tracked files: a three-line `.gitignore`, the MIT `LICENSE`, a one-line `README.md`, the brainstorm at `docs/design/peti-physical-model-brainstorm.md`, the master build plan at `docs/plans/2026-09-13-savera-build-plan.md`, and the completed rename plan in `docs/plans/done/`. At that point, `feature/phase-0` sat at `dea180f`, the same commit as `main`. Nothing in Phase 0 is Zig: a Zig CI workflow reads its toolchain from `build.zig.zon`, so `ci.yml` arrives with that file in Phase 1 rather than ending Phase 0 red.
...
1. Update the Status line of the Risks entry "gh-actions release timing" in place: resolved on 2026-09-14, because v3.2.0 ships `set-up-clap-validator`. Change nothing else in the master plan outside the new subsection. The Phase 0 row's `typos.toml` wording was already corrected in `c3ff304`, before execution began, and is not reverted.
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| P1  | A misspelled word in an ADR body, locally and in the CI plant                                                                                                           | `typos`, and the `typos` job                                         | Locally, typos exited 2 naming the word at `docs/adr/0006-reject-webview-ui.md:27:5`, and markdownlint stayed at 0 issues. CI run 34915549681: the `typos` job failed on the same word                                                                                                       | The `typos` job, on every push and pull request                                                                                               |
| P2  | One table row's pipes misaligned in a note, locally and in the CI plant                                                                                                 | Prettier `--check`, and markdownlint `MD060`, each on its own        | Locally, each caught it alone: Prettier flagged the note (exit 1) and markdownlint reported `MD060` twice on line 16 (exit 1). CI run 34915549681: markdownlint failed on `MD060`, but `Run Prettier check` was skipped, because `lint-text.yml` runs Prettier only when markdownlint passed | The `text` job's markdownlint step; Prettier's CI arm is covered by P12                                                                       |
| P3  | A relative link to a file that does not exist, in `AGENTS.md`, locally and in the CI plant                                                                              | `relative-links`                                                     | Locally, `relative-links` reported the missing file once for `AGENTS.md` and once for `CLAUDE.md`, exit 1. CI run 34915549681: the same two findings, from the rule resolved through the workspace's `node_modules`                                                                          | The `text` job                                                                                                                                |
| P4  | A link to an existing ADR with a fragment that names no heading, locally and in the CI plant                                                                            | `relative-links`, fragment check                                     | Locally, "should have a valid fragment identifier", reported for both names, exit 1. CI run 34915549681: the same                                                                                                                                                                            | The `text` job                                                                                                                                |
| P5  | Prettier over the brainstorm with `--ignore-path /dev/null`, then with the ignore file                                                                                  | Prettier; the second run must skip the file                          | Re-run in the repository: with the ignore file, skipped both when named explicitly and under `prettier --check .`; with it bypassed, flagged, exit 1                                                                                                                                         | By hand in the local gate at each phase pull request                                                                                          |
| P6  | markdownlint over the brainstorm with `docs/design/**` removed from `ignores`                                                                                           | markdownlint                                                         | Re-run in the repository: 13 issues, all in the brainstorm                                                                                                                                                                                                                                   | By hand                                                                                                                                       |
| P7  | One byte of the brainstorm changed                                                                                                                                      | Nothing in CI, by decision                                           | Prettier, markdownlint, typos and gitleaks all exited 0. The SHA-256 changed to `817c9144…`, and `git diff --exit-code` exited 1                                                                                                                                                             | By hand: the recorded SHA-256 and `git diff --exit-code`                                                                                      |
| P8  | An unquoted variable (`SC2086`) in a `run:` step of `text-lint.yml`, locally and in the CI plant                                                                        | actionlint with shellcheck                                           | Locally, `SC2086` twice on the planted line, exit 1. CI run 34915549681: `actions / actionlint` failed on the same two findings                                                                                                                                                              | The `actions` job                                                                                                                             |
| P9  | P8 again, locally, with shellcheck removed from `PATH`                                                                                                                  | Nothing; the expected result is exit 0, the vacuous pass             | actionlint exited 0 on the plant: the vacuous pass is real                                                                                                                                                                                                                                   | The pinned shellcheck install in `lint-github-actions.yml`, whose `Report tool versions` step printed ShellCheck 0.11.0 in CI run 34915386623 |
| P10 | A generated AWS-shaped access-key identifier (`AKIA` plus 16 characters drawn from `[A-Z2-7]`) in an untracked Markdown file; run `gitleaks detect --no-git --source .` | gitleaks                                                             | `aws-access-token` flagged `docs/notes/plant.md`                                                                                                                                                                                                                                             | The `gitleaks` job, over the full history on every push and pull request                                                                      |
| P11 | The same string in an untracked `fixtures/plant.wav`                                                                                                                    | Nothing; the fixture allowlist entry, expected silent                | Suppressed. The control, the same key in a root-level `plant.wav`, was flagged                                                                                                                                                                                                               | The `.gitleaks.toml` entry, which the `gitleaks` job discovers without `allowlist-config`                                                     |
| P12 | An extra space after a colon in a workflow's YAML                                                                                                                       | Prettier over YAML                                                   | Locally, Prettier flagged `gitleaks.yml` (exit 1) and actionlint exited 0. CI run 34915665708, as a second CI plant: `Run markdownlint` passed and `Run Prettier check` failed naming `gitleaks.yml`                                                                                         | The `text` job's Prettier step                                                                                                                |
| P13 | P3 and P4 again, with `customRules` removed from `.markdownlint-cli2.jsonc`                                                                                             | Nothing; expected green, proving no built-in rule was the one firing | 0 issues, exit 0. With the rule restored, 4 issues                                                                                                                                                                                                                                           | The `customRules` entry; nothing else sees a cross-file link                                                                                  |
| P14 | `CLAUDE.md` replaced by a copy of `AGENTS.md`                                                                                                                           | Nothing, by decision                                                 | Prettier, markdownlint and typos exited 0. `test -L CLAUDE.md` exited 1, and `git status` showed a `T` type change                                                                                                                                                                           | By hand: `test -L CLAUDE.md`                                                                                                                  |
| P15 | A shell script under `scripts/` with `SC2086`                                                                                                                           | The `shell` job                                                      | Not planted: Phase 0 adds no script                                                                                                                                                                                                                                                          | Phase 1's gate plants it                                                                                                                      |
| P16 | A committed AWS-shaped key pair in a throwaway local repository, scanned as `scan-for-secrets.yml` runs TruffleHog                                                      | TruffleHog                                                           | Added during 0.16, measured with TruffleHog 3.95.2, the version CI installs, and 3.97.4, identically. With `--results=verified,unknown` and no `--fail`: no output, exit 0. With unverified results shown: a reported AWS finding, still exit 0. With `--fail`: exit 183                     | Nothing. The `trufflehog` job cannot fail; filed as cboone/gh-actions#111. gitleaks covers the class                                          |

P10's alphabet is load-bearing. gitleaks 8.30.1's `aws-access-token` rule matches `AKIA` followed by 16 characters from the base32 alphabet `[A-Z2-7]`, so a candidate containing `0`, `1`, `8` or `9` is not a detectable plant at all, and its silence would read as gitleaks failing. Measured on 2026-09-14 in a scratch directory outside the repository: a `[A-Z2-7]` key was flagged in a `.md` file, in a root-level `.wav` and in `fixtures/plant.wav`, and a key containing those digits was flagged nowhere.

P11 is the row that tests the allowlist rather than trusting its regex, since `^fixtures/` would match nothing if gitleaks reported paths with a leading `./`. The same scratch measurement, with the `^fixtures/.*\.(wav|npz)$` entry in place, suppressed `fixtures/plant.wav` in both the working-tree and the git-history scans while still flagging the root-level `.wav`, and gitleaks reported the path as `fixtures/plant.wav`. The row is still re-run in this repository in 0.15; if P11 is flagged there, the regex is corrected and both P10 and P11 are re-run.
...
- The master plan is still at the `docs/plans/` root and carries the new outcomes subsection. `git diff c3ff304 -- docs/plans/2026-09-13-savera-build-plan.md` shows that subsection and the one Risks status line and nothing else. Against `origin/main` the diff also carries the Phase 0 row's `typos.toml` wording from `c3ff304`, which is expected.
...
Executed on 2026-09-14 on `feature/phase-0`, in [pull request #1](https://github.com/cboone/savera/pull/1).

| Step | Status | Commits                                                                                      |
| ---- | ------ | -------------------------------------------------------------------------------------------- |
| 0.0  | Done   | `38a83fc`, then the owner's corrections in `c3ff304` and the review fix in `7e8f9e2`         |
| 0.1  | Done   | `7cb8ce5`                                                                                    |
| 0.2  | Done   | `32e686f`                                                                                    |
| 0.3  | Done   | `87b9bd7`, then `3d084be`                                                                    |
| 0.4  | Done   | `1c48909`                                                                                    |
| 0.5  | Done   | `68dc61c`                                                                                    |
| 0.6  | Done   | `5ffdf3d`                                                                                    |
| 0.7  | Done   | `39933f6`                                                                                    |
...
| 0.13 | Done   | `837a9ea`                                                                                    |
| 0.14 | Done   | `9231ca4`                                                                                    |
| 0.15 | Done   | `e1446ce`, recording plant findings                                                          |
| 0.16 | Done   | `df70dbd`; the CI plants `3ac58ba` and `e61ff7c` with their reverts; Copilot fixes `b770b96` |
| 0.17 | Done   | this commit, then the move to `done/`                                                        |

### What landed differently

- **The owner revised the plan before execution** (`c3ff304`): the Context moved to the past tense, P10 gained a concrete key shape, and the build plan's Phase 0 row now says `typos.toml` gains words only when flagged. Review of that revision (`7e8f9e2`) narrowed P10 to the base32 alphabet gitleaks actually matches and anchored the by-hand build plan check to `c3ff304`.
- **The `bootstrap-project` finding was a misreading.** The installed 1.3.1 copy did carry stale `commands/` references, but they were removed in `c459710` (#389) together with the 1.4.0 version bump, which was installed at 23:16 UTC on 2026-09-14, after the scaffolding steps had run. No version-bump issue existed to file. The Findings section above is left as written; `docs/notes/skill-deviations.md` carries the correction.
- **Four issues were filed instead of one:** [cboone/agent-harness-plugins#431](https://github.com/cboone/agent-harness-plugins/issues/431) (`set-up-linters` lint templates skip the Markdown their tools check), [#432](https://github.com/cboone/agent-harness-plugins/issues/432) (`set-up-secret-scanning`'s duplicate `scan` job names), [cboone/gh-actions#111](https://github.com/cboone/gh-actions/issues/111) (the TruffleHog job cannot fail) and [cboone/gh-actions#112](https://github.com/cboone/gh-actions/issues/112) (`lint-text.yml` skips later tools once one fails).
- **`.gitleaks.toml` no longer allowlists `build.zig.zon`**, reversing 0.2. After Copilot's review, gitleaks 8.30.1 scanned three real Zig manifests with no allowlist and found nothing, so the entry was dead config that hid the whole manifest. It was anchored to the root first, in `33c6d80`, then removed.
- **The Prettier reformat was committed after the configuration, not before** (0.3), because the reformat depends on `.prettierrc.json`. It changed only `.claude/settings.json`, collapsing its allow list onto one line.
- **The ADR group commits carry forward links**: each of the first three contains links that resolve only in a later group's commit. Every link resolves on the final tree, which is what CI judges.
- **The `clean-up-agent-config` audit added cross-references** between `copilot-instructions.md` and the three scoped instruction files (0.12). It found the layout otherwise sound.
- **A second CI plant was needed.** `lint-text.yml` runs markdownlint before Prettier and skips Prettier once markdownlint fails, so the first plant (run 34915549681) never exercised Prettier. A Prettier-only plant (run 34915665708) did.
- **The TruffleHog gate item is a vacuous pass.** `scan-for-secrets.yml` runs TruffleHog without `--fail`, and TruffleHog exits 0 without it even when it reports a finding (P16). The job is not counted as coverage; gitleaks is the secret check that can fail.
- **Copilot reviewed `df70dbd`** with 2 threads and 7 review-body findings. Five were valid and fixed in `b770b96` (the CI lint order, and the claim that every local command runs in CI). Two were incorrect and answered in the review instructions: gitleaks discovers `.gitleaks.toml` without `allowlist-config`, and `CLAUDE.md` is a tracked symlink (git mode `120000`) rather than a missing file. The two threads asked for this Outcomes section, the filled plant table, the step 0.10 correction and the move to `done/`, which this commit and the next provide.
- **`lint-and-fix` was not run** where the `pr` and `resolve-copilot-pr-feedback` skills call for it. The project's own gate ran in its place, since `markdownlint-cli2 --fix` rewrites every file its globs match. `review-branch`'s review was kept outside the repository rather than committed under `docs/reviews/`, which this plan does not include.
...
- **The repository description** reads as Decisions states.
- **CI job durations** over the two clean runs were at most 15 seconds, recorded in `docs/notes/ci-workflows.md`, and the carried timeouts stand.

### Gaps, stated rather than counted

- **The shell job passes by finding nothing.** It is wired for Phase 1.
- **The TruffleHog job cannot fail**, until cboone/gh-actions#111 is fixed and the pin moves.
- **The brainstorm's byte identity, `AGENTS.md`'s size and the `CLAUDE.md` symlink are checked by hand**, by decision; P7 and P14 show nothing in CI would notice a change.
- **Nothing builds.** There is no `zig build`, no bundle and no `clap-validator` run, by design.
